...
int NB = 12;//number of bead want to include
...
double L = 7.29;//box size, we need to change it for difference simulation
double min = 10.0;//initial minima bead to bead distance
double minr = 10.0;
double Min = 0.0;
double Minr = 0.0;
...
FILE* Distance = fopen("Distance_MetricC.dat","w");//open pairwise distance matrix file
		...
		for(int m2 = m1+1; m2 < N ; m2++){//we just need to compute halp number of melecule molecule pair, since melecule molecule distance are symmetrical
			for(int i = 0; i < NB ; i++){//loop over all possible combination of bead to bead pair in such molecule pair
				for(int j = 0; j < NB ; j++){
					...
					rr[k] = A[m2][i][k] - A[m1][j][k];
					rr[k] = rr[k] - L*round(rr[k]/L);
					rr2 += rr[k]*rr[k];
					}
				...
				if (rr2 < minr){
				minr = rr2;//find the minima bead to bead in a molecular pair
				}
